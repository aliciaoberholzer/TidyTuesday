...
#get country data from Natural Earth R package
...
dt2 <- dt[-c(1:49),]
dt2["country"][dt2["country"] == 'United States'] <- 'USA'
...
#build the plot
cookmap <- ggplot() +
  geom_sf(data = map, aes(fill = EG.CFT.ACCS.ZS, lwd = ""), color = NA) +
  labs(
    title = "Perecentage of the popoulation with access to clean cooking fuels and technologies (2020)",
  ...
  scale_fill_distiller(
    ...
    na.value = "#ECECEC" ) +
  #hack for displaying countries that have no data 
  #this is also why I assigned a column of empty strings to line width (lwd) in geom_sf
  scale_color_manual(
    labels = "NA", 
    values=NA) +              
